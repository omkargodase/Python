...
def Travel_Files(path,Dir_Name):
...
"""
    ...
    s.login(fromaddr,'YOUR MAIL PASSWORD HERE')
...
def PrintResultInFile(dict,DirName):
    ...
    fd.close()
    Sending_Mail(File_path)
    

def Schedule_Script(Dir_Name,Dir_Name1):
    print("Scheduler starts...");

    schedule.every(5).seconds.do(lambda : Travel_Files(Dir_Name,Dir_Name1))
